...
class FTMTreeEditDistanceBase : virtual public Debug {
  ...
  public:
    ...
    template <class dataType>
    void persistenceMerging(ftm::FTMTree_MT *tree, double epsilon, double epsilon2=0){
      bool fullMerge = (epsilon == 100);
      epsilon /= 100;
      epsilon2 /= 100;
...
//std::cout << "==================== " << tree->getNumberOfNodes() << std::endl;
    ...
    ftm::FTMTree_MT* preprocessingPipeline(ftm::FTMTree_MT *&tree, double epsilonTree, double epsilon2Tree, 
                                           double epsilon3Tree, int verboseT=1){
      Timer t_proc;
    ...
    void branchDecompositionToTree(ftm::FTMTree_MT *tree){      
      ...
      //printTree(tree); printPairsFromTree<dataType>(tree, true);
      ...
      while(!queue.empty()){
        ...
        /*for(int i = 0; i < childrenScalars.size(); ++i)
          std::cout<<std::get<0>(childrenScalars[i])<<" _ "<<std::get<1>(childrenScalars[i])<<std::endl;*/
        children.clear();
        for(int i = 0; i < childrenScalars.size(); ++i)
