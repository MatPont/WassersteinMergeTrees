...
std::vector<float> treePlanarLayoutImpl(FTMTree_MT *tree, int dataTypeInt,
  ...
  getTreeBranching(tree, branching, branchingID);
    
  // Iterate through tree
  std::queue<std::tuple<idNode, int, float>> queue;
  queue.push(std::make_tuple(getRoot(tree), 0, getNumberOfLeaves(tree)));
  while(!queue.empty()){
  ...
  switch(vtkTemplate2PackMacro(idType, dataTypeInt)) {
    ttkTemplate2IdMacro(
      (/*status =*/ layoutCompute.execute<VTK_T1, VTK_T2>(
         ...
         nPoints, nEdges,
         (VTK_T2 *)scalars, nullptr, (VTK_T1 *)branches, nullptr //(VTK_T1 *)levels (float *)sizes
      )));
  }
