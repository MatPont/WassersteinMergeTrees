...
//////////////////////////////////////////////////////////////////////////////+
    /*std::vector<std::vector<dataType>> costMatrix(data_min[0].size()+1,
                                                  std::vector<dataType>(data_min[1].size()+1));
    std::cout << costMatrix.size() << " _ " << costMatrix[0].size() << std::endl;
    for(int i = 0; i < costMatrix.size()-1; ++i){
      dataType nodeIValue = std::get<6>(data_min[0][i]);
      dataType nodeIOriginValue = std::get<10>(data_min[0][i]);
      for(int j = 0; j < costMatrix[0].size()-1; ++j){
        dataType nodeJValue = std::get<6>(data_min[1][j]);
        dataType nodeJOriginValue = std::get<10>(data_min[1][j]);
        costMatrix[i][j] = std::pow(nodeIValue - nodeJValue, 2) +
                           std::pow(nodeIOriginValue - nodeJOriginValue, 2);
      }
      costMatrix[i][costMatrix[0].size()-1] = 2*std::pow(std::get<4>(data_min[0][i]), 2)/(std::pow(2, 2));
    }
    for(int j = 0; j < costMatrix[0].size()-1; ++j)
      costMatrix[costMatrix.size()-1][j] = 2*std::pow(std::get<4>(data_min[1][j]), 2)/(std::pow(2, 2));
    std::vector<matchingTuple> matchings;
    Munkres assignmentSolverMunkres;
    assignmentSolverMunkres.setInput(costMatrix.size(), costMatrix[0].size(), (void *)&costMatrix);
    ...
    /*for(int i = 0; i < numberOfInputs_; i++) {
        std::cout << "size=" << data_min[i].size() << std::endl;
        for(diagramTuple tuple : data_min[i])
