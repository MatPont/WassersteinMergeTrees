///
/// \ingroup base
/// \class ttk::MergeTreeBarycenter
/// \author XXXXX
/// \date 2020.
///
/// This module defines the %MergeTreeBarycenter class that computes the 
...
namespace ttk {
    ...
    /**
    * The MergeTreeBarycenter class that computes the 
    ...
    public:
        MergeTreeBarycenter() {
          this->setDebugMsgPrefix(
              "MergeTreeBarycenter"); // inherited from Debug: prefix will be printed at the
        ...
        void setProgressiveBarycenter(bool progressive){
          ProgressiveBarycenter = progressive;
        }
        
        void setProgressiveSpeedDivisor(double progSpeed){
          ProgressiveSpeedDivisor = progSpeed;
        }
        ...
        double getAllDistanceTime(){
          return allDistanceTime;
        }

        double getAddDeletedNodesTime(){
          return addDeletedNodesTime;
        }
        ...
        */
        ...
        void getDistanceMatrix(std::vector<ftm::FTMTree_MT*> &trees, 
          ...
          distanceMatrix = std::vector<std::vector<double>>(trees.size(), 
                                                            std::vector<double>(trees.size(), 0));
...
#pragma omp parallel for schedule(dynamic) num_threads(NumberOfThreads) if(Parallelize)
        ...
        int getBestInitTreeIndex(std::vector<ftm::FTMTree_MT *> &trees, 
                                 std::vector<ftm::FTMTree_MT *> &trees2, bool distMinimizer=true){
          ...
          int bestIndex = -1;
          dataType bestValue = distMinimizer ? std::numeric_limits<dataType>::max() : 0;
          ...
          for(int i = 0; i < trees.size(); ++i){
            ...
            if(distMinimizer){
              ...
              value = getRealNumberOfNodes(trees[i]);
              if(trees2.size() != 0)
                value += getRealNumberOfNodes(trees2[i]);
            }
            if( (distMinimizer and value < bestValue) or (not distMinimizer and value > bestValue) ){
              bestIndex = i;
              bestValue = value; 
            }
            sizes[i] = -value; 
        ...
        void updateBarycenterTreeStructure(std::vector<ftm::FTMTree_MT *> &trees, MergeTree *&baryMergeTree,
          ...
          ftm::idNode baryTreeRoot = getRoot(baryTree);
            ...
            while(!queue.empty()){
              ...
              if(matrixMatchings[i][node] == -1){ // if node in trees[i] is not matched
                if(not KeepSubtree){
                  ...
                  // TODO manage if keepSubtree=true (not important because invalid barycenter)
...
if( (isNodeIdInconsistent(baryTree, parent) or isNodeAlone(baryTree, parent)) and matchings[i].size() != 0){
  std::cout << getParent(trees[i], node) << " _ " << node << " _ " << std::endl;
  printTree(trees[i]); printPairsFromTree<dataType>(trees[i], true); printMatching(matchings[i]);
  //printTree(baryTree); printPairsFromTree<dataType>(baryTree, true);
  std::cout << "parent " << parent << std::endl;
  myPause();
}
                /*if(isRoot(trees[i], node))
                  parent = getRoot(baryTree);*/
            ...
            // todo manage if keepSubtree=true (not important since it is not a valid merge tree)
            std::cout << "ERROR barycenter with KeepSubtree=true is not implemented yet" << std::endl;
          }          
        }
...
//std::cout << "updateBarycenterTreeScalars" << std::endl;
          ftm::FTMTree_MT *baryTree = baryMergeTree->tree;
          bool isJT = isJoinTree<dataType>(baryTree);
...
//std::cout << "added" << std::endl;
            ...
            dataType nodeScalar = (isJT ? std::get<1>(newBirthDeath) : std::get<0>(newBirthDeath));
            dataType nodeOriginScalar = (isJT ? std::get<0>(newBirthDeath) : std::get<1>(newBirthDeath));
            newScalarsVector[node] = nodeScalar;
            newScalarsVector[baryTree->getNode(node)->getOrigin()] = nodeOriginScalar;
//////////////////////////////////////////
...
//std::cout << cptBug << " _/_ " << getRealNumberOfNodes(baryTree) << std::endl;
          ...
          persistenceThresholding<dataType>(baryMergeTree->tree, 0, deletedNodesT);
          if(not IsCalled)
        ...
        void computeOneDistance(ftm::FTMTree_MT *tree, ftm::FTMTree_MT *baryTree, 
          ...
          //ftmTreeEditDistance.setParallelize(false);
          ...
          /*if(ProgressiveBarycenter){
            ftmTreeEditDistance.setAuctionNoRounds(1);
            ftmTreeEditDistance.setAuctionEpsilonDiviser(NoIteration-1);
          }*/
          ...
          //allDistanceTime += t_distance_time;
        }
...
//myPause();
        ...
        template <class dataType>
        void assignmentPara(std::vector<ftm::FTMTree_MT *> &trees, MergeTree *baryMergeTree,
                            std::vector<std::vector<std::tuple<ftm::idNode, ftm::idNode>>> &matchings,
                            std::vector<dataType> &distances, int verboseT=0){
#ifdef TTK_ENABLE_OPENMP
#pragma omp parallel num_threads(NumberOfThreads) if(Parallelize)
{
#pragma omp single nowait
        ...
        template <class dataType>
        void assignmentTask(std::vector<ftm::FTMTree_MT *> &trees, MergeTree *baryMergeTree,
                            std::vector<std::vector<std::tuple<ftm::idNode, ftm::idNode>>> &matchings,
                            std::vector<dataType> &distances, int verboseT=0){
          for(int i = 0; i < trees.size(); ++i)
#ifdef TTK_ENABLE_OPENMP
#pragma omp task firstprivate(i) untied shared(matchings, distances)
#endif
...
#pragma omp taskwait
        ...
        template<class dataType>
        int persistenceScaling(std::vector<ftm::FTMTree_MT *> &trees, std::vector<MergeTree *> &mergeTrees,
                                std::vector<ftm::FTMTree_MT *> &oriTrees, int iterationNumber,
                                std::vector<std::vector<ftm::idNode>> &deletedNodes, 
                                int verboseT = 0){
          deletedNodes = std::vector<std::vector<ftm::idNode>>(oriTrees.size());
          int noTreesUnscaled = 0;

          // Scale trees
          for(int i = 0; i < oriTrees.size(); ++i){
            double persistenceThreshold = 50.0;
            if(iterationNumber != -1){
              // Get number of pairs in scaled merge tree
              int noPairs = getRealNumberOfNodes(mergeTrees[i]->tree);
              
              // Get pairs in original merge tree
              std::vector<std::tuple<ftm::idNode, ftm::idNode, dataType>> pairs;
              getPersistencePairsFromTree<dataType>(oriTrees[i], pairs, BranchDecomposition);
              
              // Compute new persistence threshold
              double multiplier = (ProgressiveSpeedDivisor < 1e-6 ? 1. : iterationNumber/ProgressiveSpeedDivisor);
              ...
              persistenceThreshold = persistence / std::get<2>(pairs[pairs.size()-1]) * 100;
              if(thresholdIndex == 0){
            ...
            if(persistenceThreshold != 0.){
              MergeTree* mt = copyMergeTree<dataType>(oriTrees[i]);
              persistenceThresholding<dataType>(mt->tree, persistenceThreshold, deletedNodes[i]);
              if(mergeTrees.size() == 0)
                mergeTrees = std::vector<MergeTree *>(oriTrees.size());
              if(iterationNumber != -1)
                freeMergeTree<dataType>(mergeTrees[i]);
              mergeTrees[i] = mt;
              trees[i] = mt->tree;
            }else{
              /*trees.clear();
        ...
        template<class dataType>
        void addScaledDeletedNodesCost(std::vector<ftm::FTMTree_MT *> &oriTrees, 
                                       std::vector<std::vector<ftm::idNode>> &deletedNodes, 
        ...
        MergeTree* computeBarycenter(std::vector<ftm::FTMTree_MT *> &trees, MergeTree *&baryMergeTree,
          ...
          ftm::FTMTree_MT *baryTree = baryMergeTree->tree;
          //treeDepth = getTreeDepth(baryTree);

          // Persistence scaling
          std::vector<ftm::FTMTree_MT *> oriTrees;
          std::vector<MergeTree *> scaledMergeTrees;
          std::vector<std::vector<ftm::idNode>> deletedNodes;
          if(ProgressiveBarycenter){ 
          ...
          while(not converged){
            ...
            if(verboseT >= 1)
              std::cout << "------------------" << std::endl << "Iteration " << NoIteration << std::endl;
            ...
            if(ProgressiveBarycenter)
              addScaledDeletedNodesCost<dataType>(oriTrees, deletedNodes, distances);
            addDeletedNodesTime += t_addDeletedNodes.getElapsedTime();
            auto t_assignment_time = t_assignment.getElapsedTime() - t_addDeletedNodes.getElapsedTime();
            if(verboseT >= 1)
            ...
            auto frechetDiff = myAbs<dataType>(frechetEnergy - currentFrechetEnergy); 
            converged = (frechetDiff <= Tol);
            converged = converged and (not ProgressiveBarycenter or treesUnscaled);
            frechetEnergy = currentFrechetEnergy;
            if(verboseT >= 1){
              std::cout << "Frechet energy : " << frechetEnergy << std::endl;
              std::cout << "TIME BARYCENTER = " << t_bary.getElapsedTime() - addDeletedNodesTime << std::endl;
            }
            Tol = frechetEnergy / 125.0;
            //std::cout << treesUnscaled << " _ " << Tol << " _ " << frechetDiff << std::endl;
            
            minFrechet = std::min(minFrechet, frechetEnergy);
            if(not converged and (not ProgressiveBarycenter or treesUnscaled) ){
            ...
            // --- Persistence scaling
            if(ProgressiveBarycenter){
              int noTreesUnscaled = persistenceScaling<dataType>(trees, scaledMergeTrees, oriTrees, NoIteration, deletedNodes, verboseT);
              treesUnscaled = (noTreesUnscaled == oriTrees.size());
            }
            
            //printTree(baryTree); printPairsFromTree<dataType>(baryTree, BranchDecomposition);
            //printTreeScalars<dataType>(baryTree); 
          }
          ...
          if(verboseT >= 1){
            ...
            std::cout << "TIME BARYCENTER = " << t_bary.getElapsedTime() - addDeletedNodesTime << std::endl;
            std::cout << "end barycenter." << std::endl;
          }
          //std::cout << "Bary Distance Time = " << allDistanceTime << std::endl;
        ...
        MergeTree* execute(std::vector<ftm::FTMTree_MT *> &trees, std::vector<double> &alphas,
          ...
          // --- Execute
          baryMergeTree = computeBarycenter<dataType>(trees, baryMergeTree, alphas, finalMatchings, 
                                                      verbose);
          ...
          if(Postprocess){
            ...
            for(int i = 0; i < trees.size(); ++i){
              allRealNodes[i] = getRealNumberOfNodes(trees[i]);
              if(allRealNodes[i] != finalMatchings[i].size())
                std::cout<<"1_matching : "<<finalMatchings[i].size()<<" / "<<allRealNodes[i]<<std::endl;
            ...
            for(int i = 0; i < trees.size(); ++i){
              convertBranchDecompositionMatching<dataType>(baryMergeTree->tree, trees[i], finalMatchings[i]);
              //printOutputMatching<dataType>(finalMatchings[i], baryMergeTree->tree, trees[i], false);
              
              allRealNodes[i] = getRealNumberOfNodes(trees[i]);
              if(allRealNodes[i] != finalMatchings[i].size())
                std::cout<<"2_matching : "<<finalMatchings[i].size()<<" / "<<allRealNodes[i]<<std::endl;
            }
          }
        ...
        template <class dataType>
        void fixMergedRootOriginBarycenter(MergeTree *&barycenter){
          if(not isFullMerge(barycenter->tree))
            return;
          ...
          if( (isJT and tree->getValue<dataType>(maxIndex) > oldOriginValue) or
              (not isJT and tree->getValue<dataType>(maxIndex) < oldOriginValue) ){
          ...
          //printTree(barycenter->tree); printPairsFromTree<dataType>(barycenter->tree); myPause();
        }
