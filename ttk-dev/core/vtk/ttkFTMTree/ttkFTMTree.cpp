...
vtkSmartPointer<vtkDataArray> createDataArray(int dataType){
...
int ttkFTMTree::getSegmentation(vtkDataSet *outputSegmentation) {
  ...
  auto scalars = outputSegmentation->GetPointData()->GetArray(ScalarField.data());
  auto newScalars = createDataArray(scalars->GetDataType());
  newScalars->DeepCopy(scalars);
  newScalars->SetName("Scalars");
  outputSegmentation->GetPointData()->AddArray(newScalars);
