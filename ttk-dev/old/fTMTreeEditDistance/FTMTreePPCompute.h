...
// Copied from FTMTreePP and adapted to FTMTreeEditDistance 
    ...
    protected:
      ...
      SimplexId getMostPersistVert(const idNode current,
        ...
        for(const auto nodeid : uf->getOpenedArcs()) {
          ...
          if((isSplitTree and tmpVal > minVal) or (!isSplitTree and tmpVal < minVal)){
          //if((isSplitTree and vtmp > minVert) or (!isSplitTree and vtmp < minVert)){
            minVal = tmpVal;
            minVert = vtmp;
          }
        }
      ...
      std::vector<ttk::ftm::idNode> getLeaves(ttk::ftm::FTMTree_MT *tree){
        ...
        for(ttk::ftm::idNode i = 0; i < tree->getNumberOfNodes(); ++i){
          if(tree->getNode(i)->getNumberOfDownSuperArcs() == 0 and 
            tree->getNode(i)->getNumberOfUpSuperArcs() != 0)
      ...
      bool getTreeType(ttk::ftm::FTMTree_MT *tree){
        ...
        for(ttk::ftm::idNode i = 0; i < tree->getNumberOfNodes(); ++i){
          if(tree->getNode(i)->getNumberOfDownSuperArcs() == 0 and 
            ...
            auto parent = tree->getSuperArc(tree->getNode(i)->getUpSuperArcId(0))->getUpNodeId();
